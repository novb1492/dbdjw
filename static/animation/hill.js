export default class hill {
  constructor(totalpoints, totalsteps, pointbundle, s) {
  ...
  gety(sheepx, pointx, pointy, pointx2, pointy2) {
    ...
    for (var i = 0; i < this.totalsteps; i++) {
      ...
      if (xyz[0] <= sheepx) {
        ...
        let a = this.s.atan2(ty, tx);///곡선의 기울기 구하는공식
        xyz[2] = a;
        return xyz;
      }
    }
  ...
  drawhills(pointx, pointy) {
    ...
    for (var ii = 0; ii < this.pointbundle; ii++) {
      for (var i = first; i > 0; i--) {
        this.s.bezier(pointx[ii][i - 1], pointy[ii][i - 1], pointx[ii][i - 1] + this.secondx, pointy[ii][i - 1] + this.secondy, pointx[ii][i] - this.secondx, pointy[ii][i] - this.secondy, pointx[ii][i], pointy[ii][i]);
      }
    }
  }
